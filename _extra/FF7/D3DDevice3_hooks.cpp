...
HRESULT __stdcall D3DDEVICE3_HOOK_DrawIndexedPrimitive(LPVOID *ppvOut, D3DPRIMITIVETYPE d3dptPrimitiveType, DWORD dwVertexTypeDesc, LPVOID lpvVertices, DWORD dwVertexCount, LPWORD lpwIndices, DWORD dwIndexCount, DWORD dwFlags) {
	...
	if(g_config.displaymode >= 0) {
		...
		if(dwVertexCount >= 1) {
			...
			float offsetX = (float)(g_currentviewport.x-g_currentviewport.old_x*g_game.mod);
			float offsetY = (float)(g_currentviewport.y-g_currentviewport.old_y*g_game.mod);
			...
			for(DWORD i = 0; i < dwVertexCount; i++) {
				...
				//vert[i].z == vert[i+1].z && vert[i].z == vert[i+2].z && vert[i].z == vert[i+3].z: !TEMP! excludes most 3d textures (change if possible)
				if(dwVertexCount%4 == 0 && vert[i].rhw != 1.0f && vert[i].z == vert[i+1].z && vert[i].z == vert[i+2].z && vert[i].z == vert[i+3].z) {
					...
					float adj = (g_config.expert_mode >= 1 ? g_config.tex_uvmap_adj_backgrounds : 0.0005f);
					if(i%4 == 0) {
						vert[i].u += adj;
						vert[i].v += adj;
					} else if(i%4 == 1) {
				...
				if(dwVertexCount%4 == 0 && vert[i].rhw == 1.0f ) { //&& vert[i-(dwVertexCount%4)+2].u-vert[i-(dwVertexCount%4)].u <= 0.0625000000f //test to fix square logo, does not work (logo is divided in same way with [u2-u=16 / v2-v=16])
					//const float adj = 0.0015f;
					float adj = (g_config.expert_mode >= 1 ? g_config.tex_uvmap_adj_interface : 0.0005f);
					if(i%4 == 0) {
						vert[i].u += adj;
						vert[i].v += adj;
					} else if(i%4 == 1) {
